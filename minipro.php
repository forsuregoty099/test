...
function makeRequest($url) {
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      //If the formAction field appears in the POST data, remove it so the destination server doesn't receive it.
      $postData = Array();
      parse_str(file_get_contents("php://input"), $postData);
      if (isset($postData["formAction"])) {
...
if (isset($_POST["formAction"])) {
  $url = $_POST["formAction"];
  unset($_POST["formAction"]);
} else {
  $queryParams = Array();
  parse_str($_SERVER["QUERY_STRING"], $queryParams);
  //If the formAction field appears in the query string, make $url start with its value, and rebuild the the query string without it.
  if (isset($queryParams["formAction"])) {
    $formAction = $queryParams["formAction"];
    unset($queryParams["formAction"]);
...
if (empty($url)) {
    if (empty($startURL)) {
      ...
      miniProxy can be directly invoked like this:
      <a href=\"" . PROXY_PREFIX . $landingExampleURL . "\">" . PROXY_PREFIX . $landingExampleURL . "
      </a>
      <br />
      <br />
      ...
      <br />
      <br />
      <form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"Proxy It!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="formAction" value="' . htmlspecialchars($action) . '" />');
